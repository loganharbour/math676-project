...
private:
  ...
  static void
  integrate_face_term(DoFInfo & dinfo1, DoFInfo & dinfo2, CellInfo & info1, CellInfo & info2);
};

template <int dim>
Problem<dim>::Problem() : mapping(), fe(1), dof_handler(triangulation)
{
}
...
Problem<dim>::assemble_system()
{
  ...
  MeshWorker::loop<dim, dim, MeshWorker::DoFInfo<dim>, MeshWorker::IntegrationInfoBox<dim>>(
      ...
      dof_info,
      info_box,
      &Problem<dim>::integrate_cell_term,
      &Problem<dim>::integrate_boundary_term,
      &Problem<dim>::integrate_face_term,
      assembler);
}

template <int dim>
void
Problem<dim>::integrate_cell_term(DoFInfo & dinfo, CellInfo & info)
{
...
template <int dim>
void
Problem<dim>::integrate_boundary_term(DoFInfo & dinfo, CellInfo & info)
{
...
template <int dim>
void
Problem<dim>::integrate_face_term(DoFInfo & dinfo1,
                                  DoFInfo & dinfo2,
                                  CellInfo & info1,
                                  CellInfo & info2)
{
  ...
  const unsigned int neighbor_dofs_per_cell = fe_face_values_neighbor.dofs_per_cell;
...
template <int dim>
void
Problem<dim>::solve(Vector<double> & solution)
{
...
template <int dim>
void
Problem<dim>::output_results(const unsigned int cycle) const
{
  {
...
main()
{
  AngularQuadrature aq(2);
  try
  {
    SN::Problem<2> dgmethod;
    dgmethod.run();
  }
